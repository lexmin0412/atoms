...
- **npm 配置一致性**：容器以 uid 1000 运行而 `/usr/local` 归 root，故把 npm 全局 prefix 指到共享卷；
  但 `NPM_CONFIG_PREFIX` 会改变 pnpm 读取**全局 npmrc** 的位置（`$PREFIX/etc/npmrc`），
  必须在容器启动时把镜像里的 npmrc 镜像过去，否则沙箱会走公网 npmjs（慢）且与 devapp 的 store 不一致
  → pnpm 判定 node_modules 需重建 → 无 TTY 直接 abort（devapp 报 503）。容器统一 `CI=true`。
...
- **归纳压缩**（`apps/api/src/compress.ts`）：模型窗口虽大（1M），但上游在 ~200k 附近就易断流，
  故设**软上限** `CONTEXT_SOFT_LIMIT`（默认 200k）；实际将发送量超过 80% 时，把「尾部 6 条之前」
  的历史交给模型归纳成一段交接摘要（已有摘要则增量更新），存 `projects.context_summary(+_count)`。
  **历史消息不删除**（用户仍能看到完整对话），只是模型看到「摘要 + 尾部」。
  摘要覆盖到压缩线附近就不再重复归纳（避免每轮都调模型）；真超软上限才立刻更新。
  实测：276k tokens → 送模型 77k。
- **模型上下文以库为准**：chat 请求只用客户端的 payload 做 UI 流；送给模型的历史由服务端按
  `messages.seq` 从库里取（客户端请求体大小不再影响模型看到的内容）。
  注意 nginx 需放行较大的请求体（前端会带上完整历史）：`client_max_body_size 32m`。
- **token 计数用标准 BPE**（`gpt-tokenizer` 的 o200k_base，懒加载 + 启发式兜底）：
  中文 1 字 ≈ 1 token（与实测一致；cl100k 会翻倍），英文/代码约 3~4 字符 1 token。
  手搓的「字符数 / 系数」曾把 234k 真实 token 估成 92k，导致压缩不触发。
  「上下文大小」取**单步输入的最大值**（多步之和会把同一份上下文重复计算）。
