...
![The agent keeps working while the generated app is already previewable in the left pane](docs/screenshots/generating-workbench.jpg)

> Live: <https://atoms.lexmin.cn> — sign up and it works (monthly credits are granted automatically).
> Try asking for: "Build an e-commerce CRM covering customers, orders, campaigns, after-sales tickets and a
> dashboard, with admin and support roles."
...
<img src="docs/screenshots/start-generate.jpg" width="100%" alt="Describe a requirement and the agent starts building">

One sentence naming the modules and roles is enough. The agent inspects the project, reads the skills that apply and
starts editing files; the preview reports **"building preview…"** and the backend readiness state right in its
header, while the top-right corner tracks **context usage / model window** and the credit balance. The preview
refreshes as build artifacts change, so the app becomes usable long before the agent finishes — every step in
between is visible as a tool card with its outcome and command output.

### What a generated app looks like

<img src="docs/screenshots/generated-result.jpg" width="100%" alt="Generated CRM app">

A full-stack CRM generated end to end: React frontend, Hono API, its own Postgres schema, role-based permissions
and seeded demo accounts — published with one click as a standalone app.
...
<img src="docs/screenshots/credits.jpg" width="100%" alt="Credits and usage ledger">

Every round is priced by token (input / output / cache, synced from models.dev) and recorded per project, so cost is
visible while you iterate. The monthly grant tops up automatically.
