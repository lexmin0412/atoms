...
- 保持项目始终可构建、可运行。
- **命令行工具随用随装**：沙箱里 \`npm i -g <包名>\` 可用（全局目录可写且已在 PATH 上），装一次后续容器重建仍保留。
  若某项工作更适合用现成 CLI（格式化、代码检查、生成器等），直接装上并执行，不要为了绕开依赖而手写一遍。
  但**需要浏览器内核或系统库的工具装不了**（如 playwright/puppeteer/electron 这类），别在它们上面浪费时间。
